...
# Default weekly roster pattern (ISO weekday 1=Mon .. 7=Sun)
# Values are canonical: OFF / OFFICE / REMOTE
...
def normalise_roster(roster) -> Dict[int, str]:
    """Normalise roster mapping to int keys 1..7 and canonical values OFF/OFFICE/REMOTE.

    JSON loads dict keys as strings; we convert to ints so persistence works across reloads.
    If the roster looks uninitialised (e.g. mostly OFF), we seed DEFAULT_ROSTER.
    """
    ...
    for k, v in roster.items():
        ...
        # Back-compat aliases
        if vv in {"WFH", "WORK_FROM_HOME"}:
            vv = "REMOTE"
        if vv in {"IN_OFFICE", "OFFICE_DAY"}:
            vv = "OFFICE"
        if vv in {"RDO", "DAY_OFF", "NONE", ""}:
            vv = "OFF"
    ...
    for i in range(1, 8):
        out.setdefault(i, "OFF")

    # If roster is effectively blank/uninitialised, seed defaults
    non_off = sum(1 for i in range(1, 8) if out.get(i, "OFF") != "OFF")
    if non_off <= 1:
        return dict(DEFAULT_ROSTER)
...
class Settings:
    ...
    roster: Dict[int, str] = field(default_factory=lambda: dict(DEFAULT_ROSTER))
...
def load_state() -> PlannerState:
    ...
    # Ensure roster persists correctly (JSON keys become strings)
    state.settings.roster = normalise_roster(state.settings.roster)
